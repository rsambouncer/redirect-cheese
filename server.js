...
function httpsReqFromURL(requrl, client_req, client_res){
    ...
    if(!qobj.hostname){
        if(client_req.headers['authorization']!=="Basic Y2hlZXNlOmNoZWVzZQ=="){ //cheese:cheese
        ...
        server_res.on('end', function(){
            if(server_res.statusCode>=300 && server_res.statusCode<400 || server_res.statusCode==201){ 
                ...
                if(type&&type.length>=9&&type.substring(0,9)==="text/html"){ 
                    ...
                    server_res.headers['content-security-policy'] = "default-src *; 'unsafe-inline' 'unsafe-eval'; frame-ancestors *";
